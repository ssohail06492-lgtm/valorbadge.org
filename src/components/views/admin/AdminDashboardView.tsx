...
export const AdminDashboardView: React.FC = () => {
  const { currentRole, setCurrentRole, setCurrentRoute, t } = useApp();
      ...
      <div className="max-w-xl mx-auto px-4 py-16 text-center space-y-6">
        ...
        <div>
          ...
          <h2 className="text-2xl font-bold text-white font-display mt-2">
            {t('Administrator Clearance Required')}
          </h2>
          <p className="text-xs sm:text-sm text-slate-300 mt-2 leading-relaxed">
            {t('The platform governance panel is restricted to verified system administrators to inspect employer CIN filings, security audit logs, and fraud reporting queues.')}
          </p>
        </div>
        <div className="flex flex-col sm:flex-row items-center justify-center gap-3">
          <button
            onClick={() => setCurrentRole('admin')}
            className="w-full sm:w-auto px-6 py-2.5 rounded-xl bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-400 hover:to-amber-500 text-slate-950 font-bold text-xs flex items-center justify-center gap-2 shadow-lg shadow-amber-500/20"
          >
            <ShieldCheck className="w-4 h-4" />
            <span>{t('Switch to Administrator Clearance')}</span>
          </button>
          <button
            onClick={() => setCurrentRoute('landing')}
            className="w-full sm:w-auto px-5 py-2.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-300 text-xs font-medium"
          >
            {t('Return to Home')}
          </button>
        </div>
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-slate-800 pb-4">
        <div>
          ...
          <h1 className="text-2xl sm:text-3xl font-extrabold text-white font-display">
            {t('ValorBadge Platform Administration & Security')}
          </h1>
          <p className="text-xs text-slate-400 mt-1">
            {t('Verify corporate employer credentials, inspect privacy compliance logs, and supervise zero-tactical data filters.')}
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="p-4 rounded-xl bg-[#071328]/80 border border-slate-800">
          <span className="text-xs text-slate-400 block mb-1">{t('Active Veterans')}</span>
          <p className="text-2xl font-bold text-white font-mono">1,480</p>
          <span className="text-[10px] text-cyan-400">{t('100% Zero-Tactical safe')}</span>
        </div>

        <div className="p-4 rounded-xl bg-[#071328]/80 border border-slate-800">
          <span className="text-xs text-slate-400 block mb-1">{t('Verified Employers')}</span>
          <p className="text-2xl font-bold text-white font-mono">124</p>
          <span className="text-[10px] text-emerald-400">{t('All CIN verified')}</span>
        </div>

        <div className="p-4 rounded-xl bg-[#071328]/80 border border-slate-800">
          <span className="text-xs text-slate-400 block mb-1">{t('Active Requisitions')}</span>
          <p className="text-2xl font-bold text-white font-mono">312</p>
          <span className="text-[10px] text-cyan-400">{t('Across 18 industries')}</span>
        </div>

        <div className="p-4 rounded-xl bg-[#071328]/80 border border-slate-800">
          <span className="text-xs text-slate-400 block mb-1">{t('Sensitive Keyword Blocks')}</span>
          <p className="text-2xl font-bold text-white font-mono">0</p>
          <span className="text-[10px] text-emerald-400">{t('All inputs compliant')}</span>
        </div>
      </div>
      ...
      <div className="flex rounded-xl bg-slate-900 p-1 border border-slate-700 text-xs font-mono w-fit">
        ...
        >
          ...
          <span>{t('Employer Approvals')} ({pendingEmployers.length})</span>
        </button>
        <button
        ...
        >
          ...
          <span>{t('Data Minimization Audits')}</span>
        </button>
        <button
        ...
        >
          ...
          <span>{t('System Audit Trail')}</span>
        </button>
      </div>
        ...
        <div className="space-y-4">
          {pendingEmployers.length === 0 ? (
            <div className="text-center py-12 p-6 rounded-2xl bg-[#071328]/80 border border-slate-800 space-y-2">
              ...
              <h3 className="text-base font-bold text-white">{t('All Employer Verifications Cleared')}</h3>
              <p className="text-xs text-slate-400">
                {t('There are no pending corporate accounts requiring review.')}
              </p>
            </div>
          ) : (
              ...
              {pendingEmployers.map(emp => (
                ...
                >
                  <div className="space-y-1">
                    ...
                    <p className="text-xs text-slate-400">
                      CIN: <span className="font-mono text-cyan-300">{emp.cin}</span> • {t('Industry:')} {t(emp.industry)}
                    </p>
                    <p className="text-xs text-slate-400">
                      {t('Contact:')} {emp.contactPerson} • {t('Submitted:')} {emp.submittedDate}
                    </p>
                  </div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    >
                      ...
                      <span>{t('Approve Verified Badge')}</span>
                    </button>
                  </div>
                </div>
              ))}
        ...
        <div className="p-6 rounded-2xl bg-[#071328]/90 border border-slate-800 space-y-4">
          <div className="flex items-center justify-between pb-3 border-b border-slate-800">
            <h3 className="text-base font-bold text-white font-display">
              {t('Zero-Tactical Compliance Guard Logs')}
            </h3>
            <span className="text-xs font-mono text-emerald-400 flex items-center gap-1">
              <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse" />
              <span>{t('Sanitization Active')}</span>
            </span>
          </div>

          <p className="text-xs text-slate-300 leading-relaxed">
            {t('ValorBadge runs client-side and server-side regular expression filters on profile fields. Any keywords relating to tactical weapon specs, deployment map coordinates, or classified unit code numbers are automatically blocked prior to persistence.')}
          </p>

          <div className="p-4 rounded-xl bg-slate-950 border border-slate-800 text-xs font-mono text-slate-400 space-y-1.5">
            <p className="text-cyan-400 font-bold">{t('// REAL-TIME COMPLIANCE METRICS')}</p>
            <p>{t('Database Records Inspected: 1,480 candidate documents')}</p>
            <p>{t('Classified Terms Intercepted: 0')}</p>
            <p>{t('Data Sovereignty Region: ap-south-1 (Mumbai, India)')}</p>
            <p>{t('Encryption at Rest: AES-256 GCM')}</p>
            <p>{t('Public Visibility Status: Restricted to Authenticated Employers')}</p>
          </div>
        </div>
      ) : (
        <div className="p-6 rounded-2xl bg-[#071328]/90 border border-slate-800 space-y-4">
          <h3 className="text-base font-bold text-white font-display pb-3 border-b border-slate-800">
            {t('Audit Activity Trail')}
          </h3>
          ...
          <div className="space-y-2">
            {DEMO_AUDIT_LOGS.map(log => (
              ...
              >
                <div>
                  <span className="font-semibold text-white">{t(log.action)}</span>
                  <p className="text-slate-400 text-[11px]">{t(log.details)} by {log.actor}</p>
                </div>
